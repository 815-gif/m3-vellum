...
window.render = (data = window.myLibrary) => {
    ...
    container.innerHTML = data.map((m, i) => {
        const realIndex = window.myLibrary.findIndex(orig => orig.title === m.title);
        return `
        <div class="manhwa-card-modern ${m.isShadow ? 'card-shadow-active' : ''}" onclick="mostrarDetalles(${realIndex})">
            <div class="card-img-wrapper">
                <img src="${m.img || 'https://via.placeholder.com/150'}" onerror="this.src='https://via.placeholder.com/150'">
                <div class="card-caps-badge">
                    <span class="cap-current">${m.current}</span>
                    <span class="cap-divider">|</span>
                    <span class="cap-total">${m.total}</span>
                </div>
                <div class="card-type-tag">${m.type || 'N/A'}</div>
                ${m.isShadow ? '<div class="shadow-icon-tag">👻</div>' : ''}
            </div>
            <div class="card-info">
                <h3>${m.title}</h3>
                <div class="card-stats-row">
                    <span class="card-rating">⭐ ${m.rating}</span>
                    <span class="card-status-dot ${m.status === 'Finalizado' ? 'status-red' : (m.status === 'Hiatus' ? 'status-orange' : 'status-green')}"></span>
                </div>
            </div>
        </div>`;
    }).join('');
...
window.procesarGuardado = function() {
    ...
    const item = {
        ...
        img: selectedImage || (editingIndex > -1 ? window.myLibrary[editingIndex].img : "https://via.placeholder.com/150"),
...
// ==========================================
// CONTROLADOR DE IMÁGENES DEL SISTEMA
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (inputFile) {
        inputFile.onchange = function(e) {
            ...
            if (file) {
                // Si la imagen pesa más de 2MB, avisamos
                if (file.size > 2 * 1024 * 1024) {
                    alert("⚠️ ¡ENERGÍA EXCESIVA! La imagen es muy pesada. Usa una menor a 2MB.");
                    return;
                }
                ...
                reader.onload = function(event) {
                    ...
                    window.selectedImage = event.target.result;
                    console.log("SISTEMA: Imagen sincronizada con éxito.");
                    
                    // Opcional: Pequeño feedback visual
                    inputFile.style.borderColor = "#00ff00";
                };
